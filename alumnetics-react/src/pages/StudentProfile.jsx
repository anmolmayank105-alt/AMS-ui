...
const StudentProfile = () => {
  ...
  const loadProfileData = useCallback(async () => {
    try {
      ...
      console.log('Profile picture from backend:', profileData?.profilePicture);
  ...
  // Enable delete button only when trimmed, case-insensitive match
  ...
  const userInitials = user ? getInitials(user.name, 'ST') : 'ST';
      ...
      <header className="relative z-10 bg-white/90 backdrop-blur-md border-b border-white/20 shadow-md">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => navigate('/dashboard/student')} className="px-3 py-2 text-sm rounded-lg text-purple-700 hover:text-purple-900 hover:bg-purple-50 transition">
              Student Dashboard
            </button>
        ...
        <section className="bg-white/90 backdrop-blur-md rounded-3xl p-6 shadow-2xl border border-white/20 hover:shadow-3xl hover:-translate-y-1 transition-all duration-300">
          <div className="flex flex-col md:flex-row md:items-center gap-6">
            ...
            <div className="flex-1">
              <h1 className="text-2xl font-extrabold text-gray-900">{user?.fullName || user?.name || 'Student Name'}</h1>
              <p className="text-gray-700">{user?.email || 'student@university.edu'} • {user?.institution?.name || user?.college || 'University Name'}</p>
              <div className="mt-3 flex flex-wrap gap-2">
                <span className="px-3 py-1 text-xs rounded-full bg-purple-100 text-purple-700 border border-purple-200">{user?.role || 'Student'}</span>
                {user?.department && (
                  <span className="px-3 py-1 text-xs rounded-full bg-blue-100 text-blue-700 border border-blue-200">{user.department}</span>
                )}
              </div>
            </div>
          </div>
            ...
            <div className="bg-white/90 backdrop-blur-md rounded-2xl p-6 shadow-xl border border-white/20 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300">
              ...
              <ul className="space-y-3 text-sm text-gray-700">
                {user?.achievements && user.achievements.length > 0 ? (
                  ...
                  <li className="text-gray-500 italic">No achievements added yet</li>
                )}
              </ul>
            </div>
          </div>
              ...
              <div className="flex flex-wrap gap-2">
                {user?.skills && user.skills.length > 0 ? (
                  user.skills.map((skill, idx) => (
                    <span key={idx} className="px-3 py-1 text-xs rounded-full bg-purple-100 text-purple-700 border border-purple-200">
                      {skill}
                    </span>
                  ))
                ) : (
              ...
              <div className="flex flex-wrap gap-2">
                {user?.interests && user.interests.length > 0 ? (
                  user.interests.map((interest, idx) => (
                    <span key={idx} className="px-3 py-1 text-xs rounded-full bg-blue-100 text-blue-700 border border-blue-200">
                      {interest}
                    </span>
                  ))
                ) : (
